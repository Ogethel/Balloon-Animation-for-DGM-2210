...
Shader "Skybox/Procedural Modified" {
Properties {
	...
	_NightColour("Night Colour", Range(0, 0.1)) = 0.003
}
...
SubShader {
	...
	Pass {
		...
		v2f vert (appdata_t v)
		{
			...
			OUT.pos = UnityObjectToClipPos(v.vertex.xyz); // sending float4, just calls function with float3
...
//				for(int i=0; i<int(kSamples); i++) // Loop removed because we kept hitting SM2.0 temp variable limits. Doesn't affect the image too much.
			...
			OUT.skyColor	= max(_Exposure * (cIn * getRayleighPhase(_WorldSpaceLightPos0.xyz, -eyeRay)), _NightColour);
